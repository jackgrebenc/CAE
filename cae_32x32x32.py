#Model source: https://github.com/alexandru-dinu/cae
...
class CAE(nn.Module):
    ...
    def __init__(self):
        ...
        # ENCODER
        #Input: b x 3 x 128 x 128 (b = batch size)

        # 64x64x64
        self.e_conv_1 = nn.Sequential(
            nn.ZeroPad2d((1, 2, 1, 2)), #b x 3 x 131 x 131
            nn.Conv2d(in_channels=3, out_channels=64, kernel_size=(5, 5), stride=(2, 2)), #b x 64 x 64 x 64
            nn.LeakyReLU()
        )

        # 128x32x32
        self.e_conv_2 = nn.Sequential(
            nn.ZeroPad2d((1, 2, 1, 2)), #b x 64 x 68 x 68
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(5, 5), stride=(2, 2)), #b x 128 x 32 x 32
            nn.LeakyReLU()
        )

        # 128x32x32
        self.e_block_1 = nn.Sequential(
            nn.ZeroPad2d((1, 1, 1, 1)), #b x 128 x 34 x 34
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)), #b x 128 x 32 x 32
            nn.LeakyReLU(),
